...
const PRESET_SERVERS = [
  ...
  { name: "Sentry", type: "local-stdio", command: "npx", args: ["-y", "@sentry/mcp-server"], env: { "SENTRY_ACCESS_TOKEN": "" }, description: "Retrieve error data, manage projects, and analyze application issues via Sentry API" },
  ...
  { name: "Insomnia", type: "local-stdio", command: "npx", args: ["-y", "mcp-insomnia"], description: "API design, debugging, and collection runner" },
  { name: "Supabase (Remote)", type: "remote-http", url: "https://mcp.supabase.com/mcp", description: "Database schema inspection, query execution, and migration management" },
  { name: "Supabase (Local)", type: "local-stdio", command: "npx", args: ["-y", "@modelcontextprotocol/server-supabase"], description: "Inspect and query local Supabase database schemas" },
  { name: "Crawl4AI", type: "local-stdio", command: "npx", args: ["-y", "crawl4ai-mcp-sse-stdio", "--stdio"], env: { "CRAWL4AI_API_ENDPOINT": "http://localhost:8000" }, description: "Web crawling and scraping with markdown optimization and structured extraction" },
  { name: "Browserbase (Remote)", type: "remote-http", url: "https://mcp.browserbase.com/mcp", description: "Cloud headless browser automation, web interaction, and extraction" },
  { name: "Browserbase (Local)", type: "local-stdio", command: "npx", args: ["-y", "@browserbasehq/mcp"], env: { "BROWSERBASE_API_KEY": "", "BROWSERBASE_PROJECT_ID": "" }, description: "Local headless browser manager and stagehand web interaction" },
  { name: "Playwright", type: "local-stdio", command: "npx", args: ["-y", "@playwright/mcp@latest"], description: "Web scraping, end-to-end testing, and browser automation via Playwright accessibility trees" },
  { name: "Netlify", type: "local-stdio", command: "npx", args: ["-y", "@netlify/mcp@latest"], description: "Deployments, serverless functions, form submissions, and Netlify hosting management" },
  { name: "Render (Remote)", type: "remote-http", url: "https://mcp.render.com/mcp", headers: { "Authorization": "Bearer " }, description: "Cloud infrastructure manager for Render services, deploys, and logs" },
  { name: "Render (Local)", type: "local-stdio", command: "npx", args: ["-y", "@niyogi/render-mcp@latest"], env: { "RENDER_API_KEY": "" }, description: "Local CLI bridge for Render infrastructure management" },
  { name: "n8n", type: "local-stdio", command: "npx", args: ["-y", "n8n-mcp"], env: { "N8N_API_URL": "", "N8N_API_KEY": "" }, description: "Control, trigger, and manage your n8n workflow automations" },
  { name: "Grafana", type: "local-stdio", command: "npx", args: ["-y", "@leval/mcp-grafana"], env: { "GRAFANA_URL": "", "GRAFANA_SERVICE_ACCOUNT_TOKEN": "" }, description: "Query Grafana metrics, Loki logs, dashboards, and alerting rules" },
  { name: "Lighthouse", type: "local-stdio", command: "npx", args: ["-y", "lighthouse-mcp"], description: "Run Google Lighthouse audits for performance, accessibility, SEO, and Core Web Vitals" },
  { name: "PageSpeed Insights", type: "local-stdio", command: "npx", args: ["-y", "@ruslanlap/pagespeed-insights-mcp"], env: { "PAGESPEED_API_KEY": "" }, description: "Google PageSpeed Insights API auditor for Core Web Vitals, metrics, and screenshots" },
  { name: "Figma", type: "remote-http", url: "https://mcp.figma.com/mcp", description: "Read Figma design tokens, component hierarchies, layouts, and write content to Figma files" },
  { name: "GTmetrix", type: "remote-http", url: "https://api.gtmetrix.com/mcp", headers: { "Authorization": "Basic " }, description: "Integrate GTmetrix API for speed audits, CrUX reports, and performance monitoring" },
  { name: "Chrome DevTools", type: "local-stdio", command: "npx", args: ["-y", "chrome-devtools-mcp@latest"], description: "Inspect, debug, profile, and interact with a live Chrome browser via Chrome DevTools Protocol" },
  { name: "Ahrefs", type: "remote-http", url: "https://api.ahrefs.com/mcp/mcp", description: "Search Ahrefs SEO metrics, crawl audits, keyword volumes, backlink parameters, and competitor analysis" },
  { name: "Builder.io DevTools", type: "local-stdio", command: "npx", args: ["-y", "@builder.io/dev-tools@latest", "mcp"], description: "Connect AI coding agents to Builder.io branches and design system documentation" },
  { name: "Builder.io CMS", type: "remote-http", url: "https://cdn.builder.io/api/v1/mcp/builder-content", headers: { "Authorization": "Bearer " }, description: "Expose Builder.io CMS content spaces, models, and entries to AI agents" },
  { name: "PlanetScale", type: "remote-http", url: "https://mcp.pscale.dev/mcp/planetscale", description: "Manage PlanetScale databases, branches, schemas, and query Insights data" },
  { name: "Flowbite / Tailwind", type: "local-stdio", command: "npx", args: ["-y", "flowbite-mcp"], description: "Access Flowbite component library snippets and Figma-to-Tailwind-code conversion assets" },
  { name: "LangSmith", type: "local-stdio", command: "npx", args: ["-y", "langsmith-mcp-server"], env: { "LANGSMITH_API_KEY": "" }, description: "Trace, debug, and optimize AI backend chains, prompts, and LLM calls in LangSmith" },
];
